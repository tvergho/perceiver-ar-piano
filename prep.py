# The code for this file is largely adapted from here: https://github.com/gwinndr/MusicTransformer-Pytorch/blob/4ea0833c8edbf8da152268f585d4f261d0cbd387/preprocess_midi.py
# And https://github.com/jason9693/midi-neural-processor
import pretty_midi
import argparse
import os
import pickle
...
# Script for preprocessing the maestro dataset.
# The dataset must be downloaded from https://magenta.tensorflow.org/datasets/maestro first.
def prep_maestro_midi(maestro_root, output_dir):
    ...
    print("Found", len(maestro_json["split"].keys()), "pieces")
    print("Preprocessing...")
    ...
    for piece in maestro_json["split"].keys():
        mid         = os.path.join(maestro_root, maestro_json["midi_filename"][piece])
        split_type  = maestro_json["split"][piece]
        ...
        total_count += 1
        if(total_count % 50 == 0):
            print(total_count, "/", len(maestro_json["split"].keys()))
...
prep_maestro_midi("./maestro-v3.0.0", "./data")